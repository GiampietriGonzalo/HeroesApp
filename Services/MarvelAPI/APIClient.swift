...
class APIClient {
    ...
    func getComicsHero(heroID: Int32, completion: @escaping ([Comic]?,Error?) -> Void){
        ...
        dataTask = urlSession?.dataTask(with: urlRequest!,completionHandler: { (data,response,error) in
            ...
            guard let dataFromResponse = data else{ return }
            
            do{
                let comicsFromResponse: ComicsResponse = try self.decoder.decode(ComicsResponse.self, from: dataFromResponse)
    ...
    func getComics(completion: @escaping ([Comic]?,Error?) -> Void ){
        ...
        dataTask = urlSession?.dataTask(with: urlRequest!,completionHandler: { (data,response,error) in
            ...
            guard let dataPosta = data else{ return }
    ...
    func getWiki(hero: Hero, completion: @escaping (String?,Error?) -> Void){
        ...
        dataTask = urlSession?.dataTask(with: urlRequest! , completionHandler: { [weak self] (data,response,error) in
            ...
            guard let dataPosta = data, let mySelf = self else { return }
